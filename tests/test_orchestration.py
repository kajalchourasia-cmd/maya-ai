...
class Stage7WorkerTests(unittest.TestCase):
    ...
    def test_nutrition_enforces_vegetarian_preference_before_generation(self):
        vegetarian = item("diet-1", ContextKind.PREFERENCE, "Vegetarian")
        choices = evidence(AgentName.NUTRITION, [
            reference("food-chicken", EvidenceLane.STRUCTURED_CATALOGUE,
                      label="Cooked chicken", tags=["nonvegetarian"], duration=30, cadence=3),
            reference("food-dal", EvidenceLane.STRUCTURED_CATALOGUE,
                      label="Vegetarian dal framework", tags=["vegetarian"], duration=30, cadence=3),
        ])
        result = self.run_worker(
            AgentName.NUTRITION, "Show meal options",
            snapshot=context(extra=[vegetarian]), worker_evidence=choices,
        )
        self.assertEqual(result.status, WorkerStatus.COMPLETED)
        self.assertEqual(result.contributions[0].item, "Vegetarian dal framework")
        self.assertEqual([citation.evidence_id for citation in result.citations], ["food-dal"])
        self.assertTrue(any("Vegetarian" in value for value in result.applied_constraints))
