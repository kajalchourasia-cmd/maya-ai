...
FOOD_PREFERENCES = {"vegetarian", "egg-friendly", "dairy-free", "gluten-free"}
FOOD_TAG_ALIASES = {
...
def _food_preferences(context: MinimalWorkerContext) -> list[ContextItem]:
    return [item for item in context.items if item.kind == ContextKind.PREFERENCE
            and item.state == FactState.CONFIRMED and isinstance(item.value, str)
            and item.value.strip().casefold() in FOOD_PREFERENCES]


def _food_candidate_allowed(reference: EvidenceReference, context: MinimalWorkerContext) -> bool:
    """Filter structured food candidates before and after the provider boundary."""

    tags = {tag.strip().casefold().replace("_", "-") for tag in reference.constraint_tags}
    for item in _constraints(context):
        if item.kind not in {ContextKind.ALLERGY, ContextKind.INTOLERANCE}:
            continue
        value = str(item.value).strip().casefold()
        forbidden = FOOD_TAG_ALIASES.get(value, {value})
        if tags & forbidden:
            return False
    preferences = {str(item.value).strip().casefold() for item in _food_preferences(context)}
    if "vegetarian" in preferences and not tags & {"vegetarian", "vegan"}:
        return False
    if "dairy-free" in preferences and tags & FOOD_TAG_ALIASES["dairy"]:
        return False
    if "gluten-free" in preferences and tags & FOOD_TAG_ALIASES["gluten"]:
        return False
    return True
...
class NutritionAgent(BoundedWorker):
    ...
    def build_draft(self, *, request_id, query, context, evidence, evaluation_only):
        ...
        usable = [reference for reference in evidence.references
                  if _food_candidate_allowed(reference, context)]
        if not usable:
        ...
        draft["applied_constraints"].extend(
            _visible_constraint(item) for item in _food_preferences(context)
        )
        draft["contributions"] = [PlanContribution(
            ...
            constraint_notes=[
                *[_visible_constraint(item) for item in _constraints(context)],
                *[_visible_constraint(item) for item in _food_preferences(context)],
                *cadence_notes,
            ],
    ...
    def validate_result(self, result, context, evidence):
        ...
        if failure:
            return failure
        expected_preferences = {_visible_constraint(item) for item in _food_preferences(context)}
        if not expected_preferences <= set(result.applied_constraints):
            return "structured_output_dropped_food_preference"
        for citation in result.citations:
            if not _food_candidate_allowed(citation, context):
