...
_SYMPTOM_LANGUAGE = re.compile(
    ...
    r"nausea|heartburn|reflux|vomit|vomiting|breath|breathing|faint|headache|vision|swelling|"
...
_PRODUCT_LANGUAGE = re.compile(
    ...
    r"record|profile|help me use|how do i use|nutrition|food|foods|eat|protein|"
    r"iron|calcium|folate|hydration|exercise|exercises|drinking water|heartburn|"
    r"self-care|self-love|wellbeing|well-being|stay active)\b"
)
...
_STORED_CONTEXT_LANGUAGE = re.compile(
    ...
    r"prior|previous|reported|history|restriction|constraint|profile)\b"
)
...
def _non_symptom_product_reason(
    ...
    if _CURRENT_EXPERIENCE.search(text):
        return None
    category = r"(?:nutrition|food|exercise|movement|wellbeing|well-being)"
    if re.fullmatch(r"(?:tell me about|explain) " + category + r"(?:(?:,? and |, )" + category + r"){0,2}[.!?]?",text):
