...
INTENT_KEYWORDS: dict[Intent, tuple[str, ...]] = {
    ...
    Intent.SYMPTOM: ("symptom", "symptoms", "pain", "bleeding", "dizzy", "breathing", "fever", "swelling", "heartburn", "nausea", "reflux"),
    Intent.NUTRITION: ("nutrition", "meal", "diet", "food", "foods", "eat", "protein", "iron", "calcium", "folate", "hydration", "drinking water"),
    Intent.MOVEMENT: ("movement", "exercise", "exercises", "fitness", "walk", "walking", "yoga", "activity", "stay active"),
    Intent.WELLBEING: ("wellbeing", "well-being", "self-love", "self love", "self-care", "grounding", "reflection", "support plan", "mental health"),
...
def build_route_plan(request: OrchestrationRequest) -> RoutePlan:
    ...
    return plan_route(
        text=request.text, safety_route=request.safety_result.route,
        requested_horizon=request.requested_horizon,
        context_kinds={item.kind for item in request.context.items},
        current_symptom=any(item.kind == ContextKind.SYMPTOM and item.current
                            for item in request.context.items),
        max_total_steps=request.max_total_steps,
    )


def plan_route(*, text: str, safety_route: str, requested_horizon: str,
               context_kinds: set[ContextKind], current_symptom: bool,
               max_total_steps: int, bounded_multi_domain: bool = False) -> RoutePlan:
    """Identity-free route planning, shared with private operator diagnostics.

    This is NOT permission to run workers. JourneyOrchestrator still validates
    authenticated context, safety provenance, evidence and runtime mode.
    """

    if safety_route != "non_urgent":
        return RoutePlan(
            intent=Intent.SYMPTOM if safety_route == "urgent" else Intent.AMBIGUOUS_MULTI_INTENT,
            selected_workers=[],
            reason=f"Stage 6 returned {safety_route}; every ordinary worker is blocked.",
            evidence_plan=[], routing_call_count=0, total_call_budget=0,
            total_step_budget=1, requires_clarification=safety_route == "needs_clarification",
        )

    intents = classify_intents(text)
    lower = text.casefold()
    plan_requested = requested_horizon != "none" or any(
    ...
    if plan_requested:
        ...
        if not contributors and (any(phrase in lower for phrase in ("full plan", "holistic plan", "weekly plan", "week plan", "daily plan", "day plan")) or (bounded_multi_domain and requested_horizon != 'none' and not intents)):
            contributors = list(PLAN_CONTRIBUTORS.values())
        contributors = list(dict.fromkeys(contributors))
        followup_context = bool(context_kinds & {
        ...
        if not followup_context:
            contributors = [agent for agent in contributors if agent != AgentName.FOLLOWUP]
        if current_symptom and not bounded_multi_domain:
            contributors = [agent for agent in contributors if agent != AgentName.MOVEMENT]
        if not contributors:
            return RoutePlan(
        ...
        return RoutePlan(
            ...
            total_step_budget=min(max_total_steps, 8 + 4 * len(selected)),
        )
    if not intents:
        return RoutePlan(
    ...
    if len(intents) > 1:
        if bounded_multi_domain and len(intents) <= 3 and all(i in PLAN_CONTRIBUTORS for i in intents):
            selected = [PLAN_CONTRIBUTORS[i] for i in intents]
            return RoutePlan(intent=Intent.MULTI_DOMAIN_EDUCATION, selected_workers=selected,
                reason="Explicit bounded educational multi-domain question; no plan is inferred.",
                evidence_plan=[EvidencePlanItem(agent=a,purpose=definition_for(a).retrieval_purpose,
                    allowed_lanes=definition_for(a).allowed_evidence_lanes) for a in selected],
                routing_call_count=0,total_call_budget=len(selected),
                total_step_budget=min(max_total_steps,8+4*len(selected)),requires_clarification=False)
        return RoutePlan(
            intent=Intent.AMBIGUOUS_MULTI_INTENT, selected_workers=[],
    ...
    return RoutePlan(
        ...
        total_step_budget=min(max_total_steps, 8),
    )
...
class JourneyOrchestrator:
    ...
    def _executed_route(route: RoutePlan, results: list[WorkerResult], reason: str) -> RoutePlan:
        return route.model_copy(update={
            ...
            "partial_execution": route.intent == Intent.FULL_PLAN,
        })
