...
class Intent(StrEnum):
    ...
    MULTI_DOMAIN_EDUCATION = "multi_domain_education"
...
class RoutePlan(Contract):
    ...
    def bounded_route(self) -> Self:
        ...
        elif len(self.selected_workers) > 1 and not (
            self.intent == Intent.FULL_PLAN and self.partial_execution
            and len(self.selected_workers) <= 4
        ) and not (
            self.intent == Intent.MULTI_DOMAIN_EDUCATION and len(self.selected_workers) <= 3
            and all(a in {AgentName.NUTRITION,AgentName.MOVEMENT,AgentName.WELLBEING,AgentName.FOLLOWUP} for a in self.selected_workers)
        ):
            raise ValueError("ordinary non-plan requests may select only one specialist")
        if self.partial_execution and self.intent != Intent.FULL_PLAN:
            raise ValueError("only a full plan may have a partial contributor route")
